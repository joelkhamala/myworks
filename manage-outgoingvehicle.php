...
if(isset($_POST['submitfree']))
{
	...
	$status = check_inputData($_POST['status']);
		$up=mysqli_query($con,"UPDATE tblvehicle SET Status='$status' WHERE Status=''");
		$upspace=mysqli_query($con,"UPDATE `spaces` SET `booked`='0',`classname`='not_booked' WHERE booked='1'");

		if($upspace)
		{
			echo "Parking is now empty";
		}
		else
		{
			echo "<p>Not freed</p>";
...
?>
              ...
              <form action="" method="POST" onsubmit ="return confirm('Are you sure? Once this change is made, there is no reverse')">
              	<input type="hidden" name="status" value="Out">
              	<button  type="submit" name="submitfree" class="btn btn-primary">Free Parking Space</button>
              </form>
