...
if(isset($_POST['submitunbook']))
{
    ...
    if($countTrans>0)
    {
        $insert = mysqli_query($con,"UPDATE spaces SET User_ID='0', booked='0', classname='not_booked',time_booked='0' WHERE Space_name='$parking'");
		$upCharge=mysqli_query($con,"UPDATE tblvehicle SET ParkingCharge='$charge' WHERE OwnerName='".$_SESSION['user']."'");
        $upTrans=mysqli_query($con,"UPDATE transactions SET space_booked='$parking' WHERE User_ID='".$userData['User_ID']."'");
		$upFreq = mysqli_query($con, "UPDATE tblusers SET parking_frequency='$park' WHERE username = '".$_SESSION['user']."'");
		$_SESSION['space_booked'] = $parking;
                          ...
                          <tr>
                          	<td colspan="2">
                          		<form action="" method="POST" id-="spaceForm">
                          			...
                          			<div class="form-row">
								        <div class="col-md-6">
                                            <button type="submit" name="submitbook" class="btn btn-success">Book P. Space</button>&nbsp;
                                        </div>
                                        <div class="col-md-6">
                                            <button type="submit" name="submitunbook" class="btn btn-info">Unbook P. Space</button>
                                        </div>
								   </div>
                          		</form>
...
{
    die("<script>alert('The system is detecting unusual behaviour, hence any further operations have been cancelled. Please Login to continue using the system.'); window.location='http://vpms.epizy.com/userAccount/login.php';</script>");
}
?>