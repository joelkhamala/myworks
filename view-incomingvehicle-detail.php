...
if(isset($_POST['submit']))
  {
    ...
    $cid=$_GET['viewid'];
     ...
     $OutTime = date('Y-m-d').' '.date('H:m:s');

 
    
     
   $query=mysqli_query($con, "update  tblvehicle set Remark='$remark',Status='$status', OutTime='$OutTime',ParkingCharge='$parkingcharge' where ID='$cid'");
    if ($query) {
    $msg="<center><p style='background-color: RGB(251,53,58); border-radius: 10px; padding: 20px; width: 50%; color: white;'>All remarks has been updated.</p></center>";
  }
  else
    {
      $msg="<center><p style='background-color: RGB(251,53,58); border-radius: 10px; padding: 20px; width: 50%; color: white;'>Something Went Wrong. Please try again.</p></center>";
    }
